...
> 2026-09-15 清理这些诊断 project 前先存档，避免数值随 project 一起消失。

| project | runs | `evaluation/return`（7500 步，除标注外） | 结论 |
|---|---|---|---|
| `rlpd_r2_smoke` | 1 | 9301.66（**25k 步**，计时校验用） | 修复前基线 |
| `rlpd_r2_repro` | 1 | 7019.88（**25k 步**，与 R1 同配置） | 发现不可复现 |
| `rlpd_r2_determinism` | 3 | 1160.11 / -386.33 / 1112.13 | 只补 `ds.seed` 后仍 FAIL（极差 1546） |
| `rlpd_r2_determinism_xla` | 3 | 226.58 / 274.68 / 354.09 | 加 XLA 确定性开关后仍 FAIL（极差 127.5） |
| `rlpd_r2_det2` | 2 | 1021.9580078125 × 2 | **两个根因都补后 PASS（极差 0.0）** |
