...
#
#   实测（2026-09-15 本机）:
#     100k 步单 run = 17.5 min（r2_A_online_s0 实测 1059 s，约 10.5 ms/步）
#     ⇒ 25 runs ≈ 7.5 h（含每 run 结束后的 wandb sync）
#
#   ⚠ 本机速度会在两档之间跳变（约 2×），原因未查明:
#     - r1 的 20 个 run 里 15 个约 270 s/25k（快档），5 个约 520 s/25k（慢档）
#     - 2026-09-15 上午的 25k 计时校验也落在慢档（~500 s）
#     - 但当天 13:53 开跑的 100k 批跑又回到快档（10.5 ms/步）
#     - 排查过: CPU governor(powersave 但实际跑到 5.15/6.6 GHz)、AC 供电(已插电)、
#       P/E 核亲和性(进程在 P 核 cpu1)、温度(48°C) —— 都不是原因
#     ⇒ 规划时按 8-14 h 估（慢档会让整批翻倍）
#
#   评估开销实测（用两个"只差评估次数"的 run 做差得出）:
#     每个 episode ≈ 0.3 s → 41 次评估 × 20 episodes ≈ 4 min/run
#
# ---------------------------------------------------------------------------
# 设计要点（沿用 r1）
# ---------------------------------------------------------------------------
...
run_group() {
  ...
  esac7.5 小时（25 runs × ~17.5 min；本机速度有 2× 的档位跳变，慢档可能 ~14 h
}
